...
// теперь мы должны объеденить шейдер и vao для этого используются атрибуты
...
// передаёт цвет дальше по ковееру (это просто надо запонить)
...
// (ниже я раскажу про матричные преобразования и библиотеку glm)
```
...
Для начала напишем код который буде рисовать четырёхугольник
...
// без матриц приобразования рисует только 2D
// и если я сейчас напишу (z = любое число), то вы не увидете разницы
...
int main()
{
    ...
    // оно и так автоматически выведится, вам лишь достаточно его поймать
	try
	{
        // инициализируем ядро
		core::Init();
        ...
        // создаём vao
        // напомню что цифра 6 - обозночает кол-во элементов на веришну
		...
		{
            // обнновляем объект событий
			window.event->update(); 

            // назначаем нашу шейдерную программу
			shader.use();

            // рисуем триугольник(точнее триугольник)
			vao.draw(TRIANGLE);
            // обратите внимание, что я не передаю кол-во вершин,
            // эта функция может их щитать автоматически
...
vert.glsl - вершиный шейдер(название и расширение можно выбрать любое)
```glsl
#version 460 core
// верчия glsl(у нас последняя как и openGL)
...
void main()
{
    ...
    // почему так? OpenGL использует четыри координаты x, y, z, w
    // w - это трансформация пока-что остовляем = 1
...
// получаем ветор из вершиного шейдера
in vec3 v_color;

// отправляем f_color дяльше по конвееру
// (это то ветор который будет обозночать цвет вершины)
// он четырёхмерный red, green, blue, alpha
out vec4 f_color;

void main()
{
    // обратите внимание alpha у нас будет = 1 пока-чтоу
...
что бы это исправить нужны матрицы преобразвания

>[!NOTE]
> Что такое матрица - это математический объект, записываемый в виде прямоугольной таблиц\
...
>
> Вектор - это(забудьте то что вам говорили на математике) структура с перемеными, хранящими 
> координаты

![](png/image-023.gif)\
вот пример умножения матрицы на ветор(таким образом\
можно вращать, перемещать, маштобировать, проецировать модель на экран)
...
2. матрица маштабирования\
![](png/i.png)\
где q - это коэффициент маштабирования по определённой оси

3. матрица перемещения\
![](png/transform.png)\
где t - значение на которое будет перемещён ветор по определёным осям
